      ...
      <div className="bg-[var(--card)] w-full max-w-md p-6 md:p-7 rounded-3xl border border-zinc-300 dark:border-zinc-800 shadow-2xl relative z-10 animate-in fade-in zoom-in duration-300 ease-out">
        ...
        {isForgotPasswordSuccess ? (
          <div className="text-center py-8 animate-in fade-in zoom-in">
            ...
            <button onClick={onClose} className="w-full bg-[var(--primary)] text-[var(--primary-foreground)] py-3 rounded-xl font-bold shadow-lg hover:opacity-90 transition-all duration-300">
              {t('auth.btnClose')}
            </button>
          </div>
        ) : (
          <>
            ...
            {feedback.text && (
              <div className={`mb-4 p-3 border rounded-xl flex items-start gap-2.5 text-xs font-bold animate-in fade-in zoom-in slide-in-from-top-2 ${
                ...
                {feedback.isSuccess ? <CheckCircle2 className="w-4 h-4 flex-shrink-0 mt-0.5" /> : <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {type !== 'forgot-password' && (
                <div className="space-y-1.5 relative">
                  <div className="flex justify-between items-center">
                    ...
                    {type === 'login' && (
                      <button type="button" onClick={() => setType('forgot-password')} className="text-[10px] font-bold text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors pr-1">
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-[var(--primary)] text-[var(--primary-foreground)] py-3 rounded-xl font-bold shadow-lg hover:opacity-90 transition-all duration-300 flex items-center justify-center gap-2 mt-2 cursor-pointer active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed text-sm">
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>{type === 'login' ? t('auth.btnLogin') : (type === 'signup' ? t('auth.btnSignup') : t('auth.btnForgotPassword'))} <ArrowRight className="w-4 h-4" /></>}
              </button>
            </form>
            ...
            <div className="mt-6 text-center text-xs text-[var(--muted-foreground)] font-medium">
              <p>
                ...
                <button type="button" disabled={isLoading} onClick={() => setType(type === 'forgot-password' ? 'login' : (type === 'login' ? 'signup' : 'login'))} className="text-[var(--foreground)] ml-1 font-bold cursor-pointer hover:underline decoration-[var(--primary)] underline-offset-4 transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-transparent border-none p-0">
